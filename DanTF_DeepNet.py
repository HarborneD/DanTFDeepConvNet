...
class DanTFDeepNet():
	...
	def Run(self,x_input):
		...
		for layer_index in range(len(self.layers)-1):
			self.RunLayer(self.W_dict,self.B_dict,y_list,self.dropout_dict,self.layers[layer_index],layer_index,inc_relu_step=True)

		print("Layer:"+str(layer_index + 1)+" (Final Layer)")
		self.RunLayer(self.W_dict,self.B_dict,y_list,self.dropout_dict,self.layers[layer_index+1],layer_index+1,inc_relu_step=False)


		return tf.argmax(y_list[-1],1)
